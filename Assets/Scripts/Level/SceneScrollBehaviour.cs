...
public class SceneScrollManager : MonoBehaviour
{
    ...
    public float minLevelScrollSpeed = 0.0f;
    ...
    private void HandleCloudMovement()
    {
        ...
        float moveAmount = -movementInput.x * (movementInput.x > 0 ? leftCloudEnlargeSpeed : leftCloudRedactSpeed) * Time.deltaTime;

        cloudPos.x = Mathf.Clamp(cloudPos.x + moveAmount, initialCloudPosition.x, initialCloudPosition.x + screenWidth / 2);
    ...
    private void OnMovePerformedReceived(UnityEngine.InputSystem.InputAction.CallbackContext ctx)
    {
        movementInput = ctx.ReadValue<Vector2>();
    }

    private void OnMoveCanceledReceived(UnityEngine.InputSystem.InputAction.CallbackContext ctx)
    {
